...
public class SphereTest {
	...
	public void getNormalTest() {
		...
		assertEquals(ap.sub(p).normalize(), sphere.getNormal(ap),
	...
	public void testFindIntersections() {
		...
		List<Point> result = sphere
				.findIntersections(new Ray(new Point(-1, 0, 0), new Vector(3, 1, 0)));
		...
		if (result.get(0).getX() > result.get(1).getX())
			result = List.of(result.get(1), result.get(0));
		assertEquals(List.of(Point.ZERO, new Point(2, 0, 0)), result,
				"Ray starts before the sphere");

		// TC14: Ray starts at sphere and goes inside (1 points)
		result = sphere.findIntersections(new Ray(Point.ZERO, new Vector(1, 0, 0)));
		assertEquals(List.of(new Point(2, 0, 0)), result, "Ray starts at sphere and goes inside");

		// TC15: Ray starts inside (1 points)
		result = sphere.findIntersections(new Ray(new Point(0.5, 0, 0), new Vector(1, 0, 0)));
		assertEquals(List.of(new Point(2, 0, 0)), result, "Ray starts inside");

		// TC16: Ray starts at the center (1 points)
		result = sphere.findIntersections(new Ray(new Point(1, 0, 0), new Vector(1, 0, 0)));
		assertEquals(List.of(new Point(2, 0, 0)), result, "Ray starts at the center");
	...
	public void testFindGeoIntersections() {
		...
		var listOfGeoPoints = sphere.findGeoIntersections(new Ray(new Point(-1, 0, 0), new Vector(3, 1, 0)), 20);
		...
		listOfGeoPoints = sphere.findGeoIntersections(new Ray(new Point(1, 0, 5), new Vector(0, 0, -1)), 5);
		...
		listOfGeoPoints = sphere.findGeoIntersections(new Ray(new Point(1, 0, 5), new Vector(0, 0, -1)), 2);
